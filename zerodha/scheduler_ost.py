...
class ZerodhaScheduler:
    ...
    def __init__(self):
        ...
        self.token_renewal_time = time.fromisoformat(self.config['zerodha']['token_renewal_time'])
        self.data_start_time = time.fromisoformat(self.config['zerodha']['data_start_time'])
        self.data_end_time = time.fromisoformat(self.config['zerodha']['data_end_time'])
        self.market_start_time = time.fromisoformat(self.config['zerodha']['market_start_time'])
        self.market_end_time = time.fromisoformat(self.config['zerodha']['market_end_time'])
        
        # Data collection duration: calculated from config times
        start_seconds = self.data_start_time.hour * 3600 + self.data_start_time.minute * 60
        end_seconds = self.data_end_time.hour * 3600 + self.data_end_time.minute * 60
        self.data_collection_duration = (end_seconds - start_seconds) / 3600  # in hours
    ...
    def _get_current_market_phase(self):
        ...
        if current_time < self.token_renewal_time:
            return "pre_token"      # Before token renewal time
        elif current_time < self.data_start_time:
            return "token_phase"    # Between token renewal and data start
        elif current_time < self.data_end_time:
            return "market_hours"   # During data collection hours
        else:
            return "post_market"    # After data collection
    ...
    async def _sleep_until_next_token_time(self):
        """Sleep until the next token renewal time (skipping weekends)."""
    ...
    def _calculate_remaining_market_time(self):
        ...
        # Calculate remaining time until data end time
    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                if self._is_weekend():
                    self.logger.info("Weekend detected, sleeping until next Monday token renewal time...")
                ...
                if phase == "weekend":
                    self.logger.info("Weekend: Sleeping until next Monday token renewal time")
                    await self._sleep_until_next_token_time()
                    continue
                
                elif phase == "pre_token":
                    self.logger.info("Pre-token phase: Waiting until token renewal time")
                    await self._sleep_until_time(self.token_renewal_time)
                    if not self.running:
                        break
                    ...
                    self.logger.info("Post-market: Sleeping until tomorrow token renewal time")
                    await self._sleep_until_next_token_time()
                    continue
                ...
                if phase_after_renewal == "token_phase":
                    ...
                    self.logger.info("Token renewal completed, waiting for data collection time...")
